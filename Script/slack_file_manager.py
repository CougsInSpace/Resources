...
if __name__ == "__main__":
    ...
    while True:
        ...
        for f in json["files"]:
            ...
            if args.delete:
                print("deleting file: '%s'" % f['id'])
                params = {'token': args.token, 'file' : f['id']}
                delete_url = API+"/files.delete"
                r = requests.post(delete_url, headers={"Authorization" : "Bearer " + args.token, "content-type" : "application/x-www-form-urlencoded"}, data=params)
