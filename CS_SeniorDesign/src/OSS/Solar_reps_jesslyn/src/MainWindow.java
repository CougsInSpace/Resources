...
implements ActionListener, IAgMsgViewerJToolBarEventsListener
{
	...
	private final static String		s_TITLE				= "CustomApp_AWT_STK_X_GfxAnalysis_SolarPanelTool";
	...
	throws Throwable
	{
		...
		cntrlJPanel.add(this.m_AgGfxAnalysisCntrlClass);
         //get rid of all gui "panel" lines
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
			...
			//}
			//else if(1==1)//(ae.getActionCommand().equalsIgnoreCase(SampleJPanel.s_STEP2_COMPUTE_TEXT))
			//{
				// data = this.m_SampleJPanel.getData();
	...
	public void doStuff()
	{
		try {
			//run scenario
			this.m_SampleCode.createScenario();
		    //set for satelite with radius of ten
			SampleData data = this.m_SampleCode.getSampleData(10);
		     //set report name as not to create multiple, but overwrite txt files
			data.ReportName = this.m_SampleCode.getReportName(data.SatelliteName, data.ReportType);
			
			//System.out.printf("name: "+data.SatelliteName + "start "+ data.StartTime + "stop "+ data.StopTime + "r "+ data.Radius + "type" +data.ReportType+ "repName"+data.ReportName+ "repTime "+ data.ReportTime);
		
		
		//compute based on data obj.
		this.m_SampleCode.compute(data);
		
		this.loadReport(data.ReportName);
		//change report type to area
		data.ReportType = "Area";
		data.ReportName = this.m_SampleCode.getReportName(data.SatelliteName, data.ReportType);
		this.m_SampleCode.compute(data);
		this.loadReport(data.ReportName);
		} catch (Throwable e) {
