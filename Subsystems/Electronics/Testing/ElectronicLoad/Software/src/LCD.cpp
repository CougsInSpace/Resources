...
static void updateBoundingBox(
    uint8_t xmin, uint8_t ymin, uint8_t xmax, uint8_t ymax) {
#ifdef enablePartialUpdate
  if (xmin < xUpdateMin)
    xUpdateMin = xmin;
  if (xmax > xUpdateMax)
...
void ST7565::drawstring(uint8_t x, uint8_t line, const char * str) {
...
void ST7565::drawchar(uint8_t x, uint8_t line, char c) {
  ...
  updateBoundingBox(
      x, line * 8, x + 5, line * 8 + 8); // only updates relevant area of LCD
}
...
void ST7565::display(void) {
  ...
  for (p = 0; p < 8; p++) {
    ...
    st7565_command(
        CMD_SET_COLUMN_UPPER | (((col + ST7565_STARTBYTES) >> 4) & 0x0F));
